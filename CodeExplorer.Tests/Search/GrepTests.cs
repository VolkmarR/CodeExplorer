...
public sealed class GrepTests : IDisposable
{
    private static readonly Dictionary<string, Dictionary<string, string>> TwoRepositories = new()
    {
        ...
        ["two"] = new Dictionary<string, string>
        {
            ...
            ["lib/wrapped.ts"] = "repo.Update(entity,\n  e => e.Status = Done);\nrepo.Update(other, e => e.Status = Open);\n",
    ...
    public async Task Text_query_matches_in_both_repositories_and_names_the_engine(SearchEngine engine, string reported)
    {
        ...
        string text = await GrepAsync(client, new() { ["query"] = "needle" });
    ...
    public async Task Regex_query_works_and_a_malformed_pattern_is_explained(SearchEngine engine)
    {
        ...
        string text = await GrepAsync(client, new() { ["query"] = "Need(le|les)\\(", ["regex"] = true, ["caseSensitive"] = true });
        Assert.Contains($"({GrepSearch.RegexEngine} engine)", text);
        Assert.Contains("one/src/Orders.cs", text);
        Assert.DoesNotContain("two/lib/index.ts", text);

        string malformed = await GrepAsync(client, new() { ["query"] = "Needle(", ["regex"] = true });
        Assert.Contains("not a valid RE2", malformed);
        Assert.DoesNotContain("No matches", malformed);

        string lookbehind = await GrepAsync(client, new() { ["query"] = "(?<=void )Needle", ["regex"] = true });
        Assert.Contains("lookbehind", lookbehind);
        Assert.Contains("RE2", lookbehind);

        string backreference = await GrepAsync(client, new() { ["query"] = "(e)\\1", ["regex"] = true });
    ...
    public async Task Filters_context_files_only_and_paging_shape_the_answer()
    {
        ...
        string excluded = await GrepAsync(client, new() { ["query"] = "Needle", ["exclude"] = "*.g.cs", ["caseSensitive"] = true });
        ...
        string context = await GrepAsync(client, new() { ["query"] = "Needle", ["path"] = "one/src/Orders.cs", ["context"] = 1 });
        ...
        string filesOnly = await GrepAsync(client, new() { ["query"] = "needle", ["filesOnly"] = true });
        ...
        string firstPage = await GrepAsync(client, new() { ["query"] = "needle", ["pageSize"] = 2 });
        Assert.Contains("page 1 of 2", firstPage);
        Assert.Contains("Call grep again with page=2", firstPage);
        string secondPage = await GrepAsync(client, new() { ["query"] = "needle", ["pageSize"] = 2, ["page"] = 2 });
        Assert.Contains("page 2 of 2", secondPage);
        Assert.DoesNotContain("Call grep again", secondPage);

        string capped = await GrepAsync(client, new() { ["query"] = "needle", ["maxLinesPerFile"] = 1 });
    ...
    public async Task Multiline_matches_span_lines_and_every_spanned_line_is_marked()
    {
        ...
        string text = await GrepAsync(client, new() { ["query"] = "Update\\([^)]*Status\\s*=", ["multiline"] = true });
        ...
        string hidden = await GrepAsync(client, new() { ["query"] = "Update\\([^)]*Status", ["multiline"] = true, ["ext"] = "cs" });
        ...
        string anchored = await GrepAsync(client, new() { ["query"] = "\\bStatus\\(", ["multiline"] = true, ["caseSensitive"] = true });
        ...
        string capped = await GrepAsync(client, new() { ["query"] = "Update\\([^)]*Status\\s*=", ["multiline"] = true, ["maxLinesPerFile"] = 1 });
    ...
    public async Task A_page_past_the_end_still_reports_the_totals(SearchEngine engine)
    {
        ...
        string text = await GrepAsync(client, new() { ["query"] = "needle", ["page"] = 7 });

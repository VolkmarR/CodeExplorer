...
| `Control__AllowLocalRepositories` | unset (`false`)                                     | Whether a local path, a `file://` URL or a remote whose host is loopback (`localhost`, 127.0.0.0/8, `::1`) may be added as a repository. Leave it off: it lets whoever may add a repository read any repository on the replica's disk or served on the replica itself. |

...
the startup log says why. A request with no Host header gets a 400 whether or not the list is set.
That default is what keeps a web page from rebinding its own name to the server's address and
reading it from a browser inside the network.
...
| A refresh skips a repository as "a local path, a file URL or a remote on a loopback host" | `Control:AllowLocalRepositories` is off, which it is by default, and has been since GHSA-5373-pppr-q3q9 for repositories stored before it; a loopback remote, such as `http://localhost/...`, is refused since #298. Point the repository at its remote, or section 5. |

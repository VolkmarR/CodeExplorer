...
any way the address can be written, though not a DNS name that resolves to the server. A
repository is not a local copy, which is the clone CodeExplorer makes of every repository.
...
The default branch is the one the remote's HEAD names. Where that HEAD is detached it names none, and
a local copy follows a branch chosen by a fixed rule instead, which the refresh status names.
